...
void main() {
  ...
  group('Drift AppDatabase tests', () {
    test(
      'Database creates tables and seeds default categories and initial wallets',
      () async {
    ...
    test(
      'Insert and query a new transaction with wallet and category references',
      () async {
    ...
    test('Insert and query a sync queue task', () async {
      ...
      final syncItem = await (db.select(
        db.syncQueueTable,
      )..where((tbl) => tbl.id.equals(syncId))).getSingleOrNull();
