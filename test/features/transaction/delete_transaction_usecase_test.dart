...
void main() {
  ...
  group('DeleteTransactionUseCase & Balance Reversal Tests', () {
    test('Deleting Expense restores wallet balance', () async {
      ...
      expect(wallet.balance, equals(2500000.0)); // Balance reversed back to 2,500,000
    });
    ...
    test('Deleting Income reduces wallet balance', () async {
      ...
      final wallet = await (db.select(db.walletsTable)..where((tbl) => tbl.id.equals('wallet_default_cash'))).getSingle();
      expect(wallet.balance, equals(2500000.0)); // Restored back
    });

    test('Deleting Transfer reverses both source and destination balances', () async {
      ...
      final bank = await (db.select(db.walletsTable)..where((tbl) => tbl.id.equals('wallet_bank_primary'))).getSingle();
      final cash = await (db.select(db.walletsTable)..where((tbl) => tbl.id.equals('wallet_default_cash'))).getSingle();
